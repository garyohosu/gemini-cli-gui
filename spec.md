...
- 以降の操作対象は**指定したフォルダ（ワークスペース）配下のみ**とし、それ以外へのアクセスは一切不可
- 以下は必ずブロック:
  - パストラバーサル（..）
  - 絶対パス指定（C:\ など）
...
- 長時間処理に備え、**サーバー側で timeout を強制**し、**キャンセル**できるようにする

### 7.6 長時間処理・中断（検証反映）
- `/prompt` は `timeoutMs` を受け取り、超過時は強制終了する
- `/prompt/start` で `requestId` を即時返却し、非同期に処理する
- `/prompt/result?requestId=...` で完了/キャンセルの結果を取得する
- `/cancel` で実行中リクエストを中断し、結果は `cancelled: true` を返す
...
#### M0-2: subprocess 連携
- [x] Python から subprocess で Gemini CLI を起動できるか → 可能
- [x] stdin/stdout の取得方法（パイプ、疑似 TTY）→ `-o json` で構造化出力
- [x] 初回起動の遅延（ウォームアップ）の計測 → 約30秒/回で安定
- [x] 連続呼び出しテスト → 3回連続で問題なし（29-33秒/回）
- [ ] 長時間実行時の挙動（タイムアウト、中断）
